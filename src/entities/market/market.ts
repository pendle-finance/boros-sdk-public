...
export class Market {
  ...
  static getInitialMarginByOrderSize(params: GetInitialMarginByOrderSizeParams, _useBuffer: boolean = true): bigint {
    const imSuf = Market.getIMSuf(params);
...
/**
 *
 * maxDeviation = factor * max(iThresh, absMarkApr)
 * @param maxRateDeviationFactorBase1e4
 * @param markApr
 * @param k_iTickThresh
 * @param tickStep
 * @returns
 * The maximum deviation of a market trade
 */
export function getMaxRateDeviation(
  maxRateDeviationFactorBase1e4: number,
  markApr: number,
  k_iTickThresh: number,
  tickStep: number
): number {
  // convert from k_iTickThresh to rate
  const iTickThresh = getRateAtTick(BigInt(k_iTickThresh), BigInt(tickStep));
  const maxRateDeviationFactor = maxRateDeviationFactorBase1e4 / 10000;
  const maxDeviation = maxRateDeviationFactor * Math.max(iTickThresh.toNumber(), Math.abs(markApr));

  // round to 3 decimal place
  return Math.floor(maxDeviation * 1000) / 1000;
}
