...
import { PendleContractErrorMessageHandler } from './PendleContractError/type';

/**
 * Convert camelCase/PascalCase to SCREAMING_SNAKE_CASE
 * Examples: MHMarketNotExists -> MH_MARKET_NOT_EXISTS
 */
...
/**
 * API-level error codes for Pendle Boros backend.
 */
export const ApiErrorCodes = {
  ...
  AMM_CUT_OFF_REACHED: 'AMM_CUT_OFF_REACHED',
  AMM_WITHDRAW_ONLY: 'AMM_WITHDRAW_ONLY',
  ...
  ORDER_WRONG_SIDE: 'ORDER_WRONG_SIDE',
  ORDER_SIZE_EXCEEDS_POSITION: 'ORDER_SIZE_EXCEEDS_POSITION',
  ...
  EIP7702_INCOMPATIBLE: 'EIP7702_INCOMPATIBLE',
  ...
  OTC_TRADE_EXPIRED: 'OTC_TRADE_EXPIRED',
  OTC_TRADE_UPDATE_FAILED: 'OTC_TRADE_UPDATE_FAILED',
...
/**
 * Contract error codes derived from the ABI.
 * Keys are original camelCase names, values are SCREAMING_SNAKE_CASE.
 */
const ContractErrorCodes = PendleContractErrorsAbi.filter((item) => item.type === 'error').reduce(
  (acc, item) => {
    const name = item.name as keyof PendleContractErrorMessageHandler;
    acc[name] = toScreamingSnakeCase(name);
    return acc;
  },
  {} as Record<keyof PendleContractErrorMessageHandler, string>
);

/**
 * Combined error codes: API errors + Contract errors
 */
