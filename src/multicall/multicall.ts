...
import { MILLISECONDS_IN_AN_HOUR, MILLISECONDS_IN_A_SECOND } from './constants';
...
export class Multicall {
  ...
  private monitor(blockTag: BlockTagMulticall, calls: readonly ContractCall[]) {
    ...
    this.multicallCache.set(blockTag.toString(), new Date());
  }
      ...
      const call = async (...params: any[]) => {
        ...
        const res = new Promise((resolve, reject) => {
          ...
          currentBatch.promise
            .then((currentResult) =>
              currentResult[dataPos] instanceof Error ? reject(currentResult[dataPos]) : resolve(currentResult[dataPos])
            )
            .catch(reject);
